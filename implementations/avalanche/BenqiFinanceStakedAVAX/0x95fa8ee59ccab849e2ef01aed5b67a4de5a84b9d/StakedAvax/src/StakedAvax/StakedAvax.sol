    ...
    StakedAvaxStorage
{
        ...
        )
    {
        ...
        for (uint i = 0; i < to.sub(from); i = i.add(1)) {
            ...
            if (paginatedUnlockRequests[i].startedAt.add(cooldownPeriod) <= block.timestamp &&
                paginatedUnlockRequests[i].startedAt.add(cooldownPeriod).add(redeemPeriod) > block.timestamp) {
    ...
    function cancelPendingUnlockRequests() external nonReentrant {
        ...
        while (unlockIndex < userUnlockRequests[msg.sender].length) {
            ...
            if (unlockRequest.startedAt.add(cooldownPeriod) <= block.timestamp) {
    ...
    function redeemOverdueShares() external nonReentrant whenNotPaused {
        ...
        while (i < unlockCount) {
            ...
            if (unlockRequest.startedAt.add(cooldownPeriod).add(redeemPeriod) >= block.timestamp) {
                i = i.add(1);
                continue;
            }
    ...
    function submit() public payable whenNotPaused returns (uint) {
        ...
        emit Transfer(address(0), sender, actualAvaxDepositAmount);
    ...
    function _redeem(uint unlockRequestIndex) internal whenNotPaused {
        ...
        require(
            unlockRequest.startedAt.add(cooldownPeriod) <= block.timestamp,
            "Cooldown period has not been completed"
        );

        require(
            unlockRequest.startedAt.add(cooldownPeriod).add(redeemPeriod) >= block.timestamp,
            "Redeem window has closed"
        );
    ...
    function _getExchangeRateByUnlockTimestamp(uint unlockTimestamp) internal view returns (bool, uint) {
        ...
        while (low <= high) {
            ...
            if (historicalExchangeRateTimestamps[mid] <= unlockTimestamp) {
                if (mid.add(1) == historicalExchangeRateTimestamps.length ||
                    historicalExchangeRateTimestamps[mid.add(1)] > unlockTimestamp) {
