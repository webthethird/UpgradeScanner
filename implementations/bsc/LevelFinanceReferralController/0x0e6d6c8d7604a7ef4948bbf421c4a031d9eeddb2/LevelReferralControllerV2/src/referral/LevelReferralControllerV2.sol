...
contract LevelReferralControllerV2 is Initializable, OwnableUpgradeable {
    ...
    function setReferrer(address _trader, address _referrer) external {
        require(msg.sender == orderHook, "LevelReferralController::setReferrer: !onlyOrderHook");
    ...
    function setOrderHook(address _orderHook) external onlyOwner {
        require(_orderHook != address(0), "LevelReferralController::setOrderHook: invalid address");
        orderHook = _orderHook;
        emit OrderHookChanged(_orderHook);
    }
    ...
    event OrderHookChanged(address orderHook);
}
