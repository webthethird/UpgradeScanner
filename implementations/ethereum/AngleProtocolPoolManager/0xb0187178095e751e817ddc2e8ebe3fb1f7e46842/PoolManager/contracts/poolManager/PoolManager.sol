    ...
    */
    ...
    function recoverERC20(
        ...
        if (tokenAddress == address(token)) {
            ...
            (, ISanToken sanToken, , , , uint256 sanRate, , , ) = IStableMaster(stableMaster).collateralMap(
                IPoolManager(address(this))
            );

            // Checking if there are enough reserves for the amount to withdraw
            require(_getTotalAsset() >= amountToRecover + (sanToken.totalSupply() * sanRate) / BASE_TOKENS, "66");
    ...
    /// @notice Forces a withdrawal from a strategy (in case notably of a loss of funds from within a strat)
    function forceStrategyWithdraw(IStrategy strategy, uint256 amount) external onlyRole(GOVERNOR_ROLE) {
        StrategyParams storage params = strategies[address(strategy)];
        params.totalStrategyDebt = params.totalStrategyDebt - amount;
        totalDebt = totalDebt - amount;
        emit StrategyReported(address(strategy), 0, 0, amount, params.totalStrategyDebt);
    }

    /// @notice Enables governance to acknowledge a loss
    function accumulateInterestOrSignalLoss(uint256 value, bool gainOrLoss) external onlyRole(GOVERNOR_ROLE) {
        if (gainOrLoss) {
            stableMaster.accumulateInterest(value);
        } else {
            stableMaster.signalLoss(value);
        }
    }
