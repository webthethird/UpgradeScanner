...
contract GlobalPool_R44 is IGlobalPool, Lockable, Pausable {
...
//        }
    ...
    */
    ...
    function removePendingUnstake(address[] calldata claimers) external onlyOwner {
        for (uint256 i = _pendingUnstakeGap; i < _pendingUnstakeClaimers.length; i++) {
            address claimer = _pendingUnstakeClaimers[i];
            uint256 toDistribute = _pendingUnstakeRequests[i];
            if (claimer == address(0) || toDistribute == 0) {
                i++;
            ...
            bool exist = false;
            for (uint j; j < claimers.length; j++) {
                if (claimers[j] == claimer) {
                    exist = true;
                    break;
                }
            }
            ...
            require(!isMarkedForManualClaim(claimer), "GlobalPool: cannot remove manual claimer");

            // refund pool
            _aethContract.mint(address(this), _aethContract.bondsToShares(toDistribute));

            emit PendingUnstakeRemoved(claimer, toDistribute);
        }
