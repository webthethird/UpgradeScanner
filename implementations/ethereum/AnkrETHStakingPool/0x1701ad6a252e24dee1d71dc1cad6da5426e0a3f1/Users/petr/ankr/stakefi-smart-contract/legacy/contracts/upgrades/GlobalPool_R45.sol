...
contract GlobalPool_R45 is IGlobalPool, Lockable, Pausable {
    ...
    mapping(address => uint256) private _aETHProviderRewards; // deleted slot:323
    ...
    function stake() public whenNotPaused("stake") nonReentrant payable {
    ...
    function stakeAndClaimAethB() public whenNotPaused("stake") nonReentrant payable {
        _stake(msg.sender, msg.value, LockStrategy.Claimable);
        claimFETH();
    }
    ...
    function _stake(address staker, uint256 value, LockStrategy lockStrategy) internal {
        ...
        if (lockStrategy == LockStrategy.Claimable) {
            ...
            revert("provider strategy not supported");
    ...
    /// @dev deprecated
    function legacyClaimableAETHRewardOf(address staker) public view returns (uint256) {
...
//        }
    ...
    */
    ...
    function removePendingUnstake(address[] calldata claimers, uint256[] calldata amounts) external onlyOwner {
        require(claimers.length == amounts.length, "GlobalPool: wrong length");

        for (uint256 i; i < claimers.length; i++) {
            address claimerToRemove = claimers[i];
            uint256 amountToRemove = amounts[i];

            for (uint256 j = _pendingUnstakeGap; j < _pendingUnstakeClaimers.length && amountToRemove != 0; j++) {
                address claimer = _pendingUnstakeClaimers[j];
                uint256 toDistribute = _pendingUnstakeRequests[j];
                // skip empty
                if (claimer == address(0) || toDistribute == 0) {
                    continue;
                }
                // match claimer
                if (claimer != claimerToRemove) {
                    continue;
                }
                // skip if manual
                if (isMarkedForManualClaim(claimer)) {
                ...
                if (toDistribute <= amountToRemove) {
                    _pendingClaimerUnstakes[claimer] = _pendingClaimerUnstakes[claimer].sub(toDistribute);
                    _pendingTotalUnstakes = _pendingTotalUnstakes.sub(toDistribute);
                    delete _pendingUnstakeClaimers[j];
                    delete _pendingUnstakeRequests[j];
                    amountToRemove = amountToRemove.sub(toDistribute);
                    // refund pool
                    _aethContract.mint(address(this), _aethContract.bondsToShares(toDistribute));
                    emit PendingUnstakeRemoved(claimerToRemove, toDistribute);
                } else {
                    _pendingClaimerUnstakes[claimer] = _pendingClaimerUnstakes[claimer].sub(amountToRemove);
                    _pendingTotalUnstakes = _pendingTotalUnstakes.sub(amountToRemove);
                    _pendingUnstakeRequests[j] = _pendingUnstakeRequests[j].sub(amountToRemove);
                    _aethContract.mint(address(this), _aethContract.bondsToShares(amountToRemove));
                    emit PendingUnstakeRemoved(claimerToRemove, amountToRemove);
                    amountToRemove = 0;
                }
            }
        }
