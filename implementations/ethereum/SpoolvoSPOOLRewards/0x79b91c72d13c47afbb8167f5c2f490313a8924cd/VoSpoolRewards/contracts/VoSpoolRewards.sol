...
contract VoSpoolRewards is SpoolOwnable, IVoSpoolRewards {
	...
	/**
	 * @notice Manually set rewards for users.
	 * @dev
	 * Manually set user rewards.
	 *
	 * Requirements:
	 *
	 * - the caller must be the Spool DAO
	 * - users count must be more than 0
	 * - users and amounts count must be of same lengt
	 *
	 * @param users Users to set rewards to
	 * @param amounts User reward amounts
	 */
	function manuallySetUserRewards(address[] calldata users, uint248[] calldata amounts) external onlyOwner {
		require(users.length > 0, "VoSpoolRewards::setUserRewards: Cannot set zero users");
		require(users.length == amounts.length, "VoSpoolRewards::setUserRewards: Users and amounts should be of the same length");

		for(uint256 i; i < users.length; ++i) {
			userRewards[users[i]].earned += amounts[i];
			emit UserRewardUpdated(users[i], 0, amounts[i]);
		}
	} 
